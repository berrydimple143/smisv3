        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form>                    
                    ...
                    @if($showOptions)
                        <div class="form-row">   
                            <div class="col-md-12 mb-3">{{ $selectLabel }}</div>
                        </div>
                        <div class="form-row">                                                    
                            @foreach($listOflevels as $level)   
                                <div class="col-md-4 mb-3">                                 
                                    <div class="form-check">
                                      <input wire.model="selectedCourses" wire:click="$emit('levelClicked', {{ $level->id }})" class="form-check-input" type="checkbox" name="levels{{ $level->id }}" id="levels{{ $level->id }}" value="{{ $level->id }}">
                                      <label class="form-check-label" for="levels{{ $level->id }}">
                                        {{ $level->name }}
                                      </label>
                                    </div>     
                                </div>                               
